#!/usr/bin/env python3
"""Plan and track synchronized minimalist Android/database releases."""
...
def make_plan(app_repo: Path, database_repo: Path, action: str) -> dict[str, Any]:
    state_path = app_repo / ".release/minimalist-state.json"
    ...
    promote_only = (
        ...
        and current_channel == "prerelease"
    )
    no_op = (
...
def render_status(plan: dict[str, Any]) -> str:
    ...
    if plan["source_changed"]:
        version_note = (
            ...
            f"(a semantic +0.1/minor bump from {plan['current_version']})."
        )
    ...
    return f"""# Minimalist release status

This file is generated from the tracked Android and database source content.
Generated manifest/state files are excluded from change detection.
...
- **build-only**: builds a paired no-image database artifact and minimalist debug APK. It does not reserve the version.
- **prerelease**: publishes both paired artifacts as GitHub prereleases when source changes exist.
...
The Android version code can still increase during prerelease-to-stable promotion because Android requires a higher internal build number for an installable update. The user-facing version name does not increase unless source content changed.
"""
